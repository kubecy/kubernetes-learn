...
~~~yaml
## 1.初始化实例对象
root@A-YYYPT-K45-M01 /root # cat kubernetes_secure_role.yaml                                                                       
apiVersion: rbac.authorization.k8s.io/v1
kind: Role
metadata:
  name: myrole
...
## 2.限定用户只能访问命名空间资源
root@A-YYYPT-K45-M01 /root # kubectl create rolebinding super-rolebind --role=myrole --user=kubecy
~~~

### 2.2.查看

![image-20240913142436524](./000.picture/image-20240913142436524.png)

~~~shell
root@A-YYYPT-K45-M01 /root # kubectl  get pod --kubeconfig=/tmp/kubecy.conf
NAME                                      READY   STATUS    RESTARTS      AGE
nfs-client-provisioner-58dcc897b4-f57pv   1/1     Running   2 (19h ago)   47h
web-0                                     1/1     Running   0             47h
web-1                                     1/1     Running   0             47h

root@A-YYYPT-K45-M01 /root # kubectl  get svc --kubeconfig=/tmp/kubecy.conf
...
~~~yaml
root@A-YYYPT-K45-M01 /root # kubectl delete rolebindings super-rolebind
~~~
